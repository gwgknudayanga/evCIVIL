...
def get_args_parser(add_help=True):
    ...
    #parser.add_argument('--dataset_parent_folder',type=str,default="/dtu/eumcaerotrain/data/Image_Data/yolo_nas_img_dataset")
    parser.add_argument('--input_img_type',type=int,default=0,help="0 img , 1 ev frame, 2 ev histogram")
    parser.add_argument('--num_input_channels',type=int,default=2,help="number of input arguments")
    parser.add_argument("--dataset_root",type=str,default="/dtu/eumcaerotrain/data/latest_dataset_encode_benchmark_25_25", help="dataset root ")
    parser.add_argument("--save_dir",type=str,default="",help="to save tenosrboard logs and ckpt files")

    return parser


##To change for events : yaml file , input_img_type arg and dataset_parent_folder arg
...
def main(args):
    ...
    print("PPPPPPPPPPPPPPPPPPPPPPPPPPPPPP ",args.rank, "  ",args.local_rank)
    
    cfg, device, args = check_and_init(args)
    # reload envs because args was chagned in check_and_init(args)
    args.local_rank, args.rank, args.world_size = get_envs()
    print("QQQQQQQQQQQQQQQQQQQQQQQQQQQ ",args.rank," ",args.local_rank," ",args.world_size)
...
if __name__ == '__main__':
    ...
    """args.dataset_root = "/media/udayanga/data_2/latest_dataset_2/latest_dataset/"
    args.input_img_type = 2
    args.num_input_channels=2
    args.save_dir = "/media/udayanga/data_2/YOLOv6/run_logs"
    args.data_path = "./data/defects.yaml"
    args.batch_size=2"""
    print("current configuration file is ",args.conf_file)
    main(args)
