...
class Evaler:
    def __init__(self,
        ...
        assert do_pr_metric or do_coco_metric, 'ERROR: at least seload_checkpointt one val metric'
        self.data = data
    ...
    def init_model(self, model, weights, task):
        if task != 'train':
            ...
            LOGGER.info("Model Summary: {}".format(get_model_info(model, self.img_size,self.args.num_input_channels)))
        if self.device.type != 'cpu':
            model(torch.zeros(1, self.args.num_input_channels, self.img_size, self.img_size).to(self.device).type_as(next(model.parameters())))
    ...
    def init_data(self, dataloader, task):
        ...
        if task != 'train':
            ...
            """dataloader = create_dataloader(self.data[task if task in ('train_img', 'val_img', 'test_img') else 'val_img'],self.data[task if task in ('train_ann', 'val_ann', 'test_ann') else 'val_ann'],
                                           self.img_size, self.batch_size, self.stride, hyp=eval_hyp, check_labels=True, pad=pad, rect=rect,
                                           data_dict=self.data, task=task, specific_shape=self.specific_shape, height=self.height, width=self.width,args=self.args)[0]"""
            
            
            from yolov6.utils.config import Config
            cfgg = Config.fromfile("./configs/yolov6n.py")
            dataloader = create_dataloader(self.data[task if task in ('train_img', 'val_img', 'test_img') else 'val_img'],self.data[task if task in ('train_ann', 'val_ann', 'test_ann') else 'val_ann'], self.args.img_size, self.args.batch_size, self.stride,hyp=dict(cfgg.data_aug), rect=False, rank=-1, pad=0.5,data_dict=self.data, task='val',specific_shape=self.args.specific_shape, height=self.args.height, width=self.args.width,args=self.args)[0]
    ...
    def eval_trt(self, engine, stride=32):
        ...
        for imgs, targets, paths, shapes in pbar:
            ...
            # preprocess
            imgs = imgs.float()
            if self.args.input_img_type == 0 or self.args.input_img_type == 1:
                imgs /= 255
