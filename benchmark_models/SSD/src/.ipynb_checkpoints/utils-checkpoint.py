...
class Encoder(object):
    ...
    def decode_batch(self, bboxes_in, scores_in, nms_threshold=0.45, max_output=200,conf_thres = 0.05):
        bboxes, probs = self.scale_back_batch(bboxes_in, scores_in)
        output = []
        for bbox, prob in zip(bboxes.split(1, 0), probs.split(1, 0)):
            bbox = bbox.squeeze(0)
            prob = prob.squeeze(0)
            output.append(self.decode_single(bbox, prob, nms_threshold, max_output,max_num=200,conf_thres = conf_thres))
        return output

    def decode_single(self, bboxes_in, scores_in, nms_threshold, max_output, max_num=200,conf_thres = 0.05):
        bboxes_out = []
        scores_out = []
        labels_out = []
        ...
        """print("bboxes_out device ",bboxes_out.device)
        print("labels_out device ",labels_out.device)
        print("scores_out device ",scores_out.device)"""
        labels_out = labels_out.to(bboxes_out.device)
...
def generate_dboxes(model="ssd"):
    ...
    #For a given feature map, there exists a on scale 
    if model == "ssd":
        figsize = 300
        #feat size means feature size
        #
        feat_size = [38, 19, 10, 5, 3, 1]
        #steps = fig_size/feature_size
        steps = [8, 16, 32, 64, 100, 300]
        #scales and aspect ratios are common for every fig size, whether it is 300x300 or 512x512
