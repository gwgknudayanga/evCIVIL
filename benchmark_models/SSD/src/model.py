...
class ResNet(nn.Module):
    def __init__(self,num_input_channels = 3):
        ...
        new_input_layer = nn.Conv2d(num_input_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.out_channels = [1024, 512, 512, 256, 256, 256]
        self.feature_extractor = nn.Sequential(new_input_layer,*list(backbone.children())[1:7])
...
class MobileNetV2(nn.Module):
    def __init__(self):
        ...
        new_input_layer = ops.Conv2dNormActivation(in_channels = 2,out_channels = 32,kernel_size = 3, stride = 2,padding = 1,bias = False,inplace = True)
        self.feature_extractor = nn.Sequential(new_input_layer,*list(self.feature_extractor.children())[1:])
