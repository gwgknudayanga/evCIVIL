...
class TrainValDataset(Dataset):
    ...
    def __init__(
        ...
        self.target_ann_save_path = self.data_dict["anno_path"]
        print("img_data_csv ",img_data_csv)
        print("ann_data_csv ",ann_data_csv)
        self.img_paths, self.labels = self.get_imgs_labels(img_data_csv,ann_data_csv,args)
        self.rect = rect
    ...
    def __getitem__(self, index):
        ...
        if dummy:
            ...
            print("Afterrrrrrrrrrrrrrrrrrrrrrr loading ", (h0, w0), " ", (h,w))
    ...
    def load_image(self, index, shrink_size=None):
        ...
        try:
            ...
            if self.args.input_img_type == 0:
                ...
                im = cv2.imread(path)
            elif self.args.input_img_type == 1:
                
                """if self.cache_ram and self.imgs[index] is not None:
                    im = self.imgs[index]
                    im = copy.deepcopy(im)
                else:"""
                #print("key printing ", [key for key in np.load(path).keys()])
                im = np.load(path)["frame_img"] #cv2.imread(path)
                
                im = np.array(Image.fromarray(im).convert('RGB'), dtype=np.uint8)
                print("im array shapeeeeee ",im.shape)
                    
            elif self.args.input_img_type == 2:
                ...
                data = np.load(path)
                im = np.load(path)["time_base_evframe"] #cv2.imread(path) #data["ev_color_img"]
                #events = data["events"]
                #im = make_dvs_frame(events, height=260, width=346, color=True, clip=None,forDisplay = False)
                #print("loading ev image")
                if "ev_color_img" in data.keys():
                    max_val = np.abs(im).max()
                    im /= (max_val + 0.0)
                    
                
            assert im is not None, f"opencv cannot read image correctly or {path} not exists"
        except Exception as e:
            print("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk ",args.input_img_type)
            print(e)
        ...
        print("ccccccccccccccccccccc ",(h0,w0)," ",im.shape)
    ...
    @staticmethod
    def generate_coco_format_labels_custom(save_path,img_paths_list,anns_for_images_list,class_names,args):
        ...
        for i in range(len(img_paths_list)):  # (img_path, info)
            ...
            if args.input_img_type == 0:
                im = cv2.imread(img_paths_list[i])
                width = im.shape[1]
                height = im.shape[0]
            elif args.input_img_type == 1:
                im = np.load(img_paths_list[i])["frame_img"]
                width = im.shape[1]
                height = im.shape[0]
            elif args.input_img_type == 2:
                im = np.load(img_paths_list[i])["time_base_evframe"] #np.load(img_paths_list[i])["ev_color_img"] time_base_evframe
                width = im.shape[1]
                height = im.shape[0]
    ...
    def get_imgs_labels(self, img_data_csv,ann_data_csv,args):
        ...
        with open(img_data_csv,"r") as f1:
            ...
            img_file_names = [os.path.join(args.dataset_parent_folder,file.rstrip()) for file in img_file_names]
            if args.input_img_type > 0:
        ...
        if args.input_img_type == 0: 
            with open(ann_data_csv,"r") as f2:
                ...
                for ann_file in ann_file_names:
                    ann_file = os.path.join(args.dataset_parent_folder,ann_file.rstrip())
        ...
        if self.task.lower() == "val":
            TrainValDataset.generate_coco_format_labels_custom(self.target_ann_save_path,img_file_names,anns_list_for_images,self.class_names,self.args)
