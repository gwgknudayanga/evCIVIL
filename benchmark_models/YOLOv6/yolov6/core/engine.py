...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.args.input_img_type = args.input_img_type  #2    #0#2
        self.args.num_input_channels = args.num_input_channels #2 #3
        self.args.dataset_parent_folder = args.dataset_root #"/dtu/eumcaerotrain/data/latest_dataset/"
        self.save_dir = self.args.save_dir #"/dtu/eumcaerotrain/data/Image_Data/yolo_nas_img_dataset/checkpoint_12th_Feb_train_day_and_night_evaluate_day_events/" #args.save_dir
        self.data_dict = load_yaml(args.data_path)
        self.num_classes = self.data_dict['nc']
        
        
        #"/dtu/eumcaerotrain/data/Image_Data/yolo_nas_img_dataset/" #"/dtu/ "/dtu/eumcaerotrain/data/NGA_Dataset/eumcaerotrain/data/NGA_Dataset/"
        ...
        print("current model deviceeeeeeeeeeee ",device)
        #checkpont_to_load_keys_name = "/dtu/eumcaerotrain/data/Image_Data/yolo_nas_img_dataset/checkpoint_9th_Feb_field_evimg/best_ckpt.pt"
        #load_model_for_available_keys(model,torch.load(checkpont_to_load_keys_name)  ['model'].float().state_dict(),device) #/work3/kniud/object_detection/YOLOv6/runs/train/field_imgs/196_ckpt.pt
        #model.load_state_dict(torch.load("/work3/kniud/object_detection/YOLOv6/runs/train/exp6/196_ckpt.pt")['model'].float().state_dict())
    ...
    def train_in_steps(self, epoch_num, step_num):
        images, targets = self.prepro_data(self.batch_data, self.device,self.args.input_img_type)
        ...
        if self.main_process:
            ...
            if is_val_epoch:
                ...
                if self.ap >= self.best_ap:
                    print("saving upto now best checkpoint ")
                    ckpt = {
                            'model': deepcopy(de_parallel(self.model)).half(),
                            'ema': deepcopy(self.ema.ema).half(),
                            'updates': self.ema.updates,
            ...
            """if self.epoch >= 10:            #self.max_epoch - self.args.stop_aug_last_n_epoch:
    ...
    def eval_model(self):
        if not hasattr(self.cfg, "eval_params"):
            results, vis_outputs, vis_paths = eval.run(self.data_dict,
                            ...
                            height=self.height,
                            verbose=True,                        
                            width=self.width,args=self.args          
                            )
        else:
            ...
            results, vis_outputs, vis_paths = eval.run(self.data_dict,
                            ...
                            model=self.ema.ema if self.args.calib is False else self.model,
                            conf_thres= 0.02,    #get_cfg_value(self.cfg.eval_params, "conf_thres", 0.03),
                            iou_thres=0.4,                           
                            dataloader=self.val_loader,
                            save_dir=self.save_dir,
                            task='train',
    ...
    def get_data_loader(args, cfg, data_dict):
        ...
        train_img_csv,train_ann_csv,val_img_csv,val_ann_csv = data_dict['train_img'], data_dict['train_ann'],data_dict['val_img'],data_dict['val_ann']
        # check data
        nc = int(data_dict['nc'])
        ...
        train_loader = create_dataloader(train_img_csv,train_ann_csv,args.img_size, args.batch_size // args.world_size, grid_size,
                                         ...
                                         cache_ram=args.cache_ram,args = args)[0]
        # create val dataloader
        val_loader = None
        if args.rank in [-1, 0]:
             # TODO: check whether to set rect to self.rect?
            val_loader = create_dataloader(val_img_csv,val_ann_csv, args.img_size, args.batch_size // args.world_size * 2, grid_size,
    ...
    @staticmethod
    def prepro_data(batch_data, device,input_img_type):
        if input_img_type == 0 or input_img_type == 1:
    ...
    def get_model(self, args, cfg, nc, device):
        if 'YOLOv6-lite' in cfg.model.type:
            ...
            model = build_lite_model(cfg, self.args.num_input_channels, nc, device)
        else:
            model = build_model(cfg, nc, device,fuse_ab=self.args.fuse_ab, distill_ns=self.distill_ns,num_input_channels=self.args.num_input_channels)
    ...
    def get_teacher_model(self, args, cfg, nc, device):
        ...
        model = build_model(cfg, nc, device, fuse_ab=teacher_fuse_ab,num_input_channels=self.args.num_input_channels)
