...
def select_subset_from_h5_file(original_h5_frame_path,original_h5_event_path,original_h5_imu_path,destination_h5_file_folder_path,desired_start_frame_number,number_of_desired_frames = -1,check_non_monotomic_only = False):
    ...
    if len(desired_frames_h5_subset.shape) < 4:
        ...
        target_frame_dataset = target_frame_h5_file.create_dataset("frames",shape=(len(desired_frames_h5_subset),260,346,3),maxshape=(None, 260,346,3),dtype="uint8")
    ...
    imu_database = target_imu_h5_file.create_dataset("imu",shape=(len(desired_imu_h5_subset),6),maxshape=(None,6),dtype="float32")
...
if __name__ == "__main__":
    ...
    original_src = "/media/udayanga/data_2/Final_Data_set/To_Labelled/Event_data_6_sep_30/h5_files/spalling/"
    subfiles = os.listdir(original_src)
    
    for sub_dir in subfiles:
        #sub_dir = "spalling_1"

        base_path = original_src + sub_dir
        original_h5_frame_path =  base_path + "/"  + sub_dir + "_frames.h5"
        original_h5_event_path = base_path + "/"  + sub_dir + "_events.h5"
        original_h5_imu_path = None #base_path + "/"  + sub_dir + "_imu.h5"

        destination_h5_file_folder_path = original_src + "/../../h5_sub_files/spalling/" + sub_dir
